...
- **Slug:** `wordpress-wp2shell-mass-exploitation`
- **Must-know:** no
...
- **Slug:** `servicenow-ai-platform-flaw-exploited-rce`
- **Must-know:** no
...
- **Slug:** `kaspersky-project-cav3rn-cyberespionage`
- **Must-know:** no
...
- **Slug:** `zimbra-patches-critical-vulnerabilities`
- **Must-know:** no
...
- **Slug:** `windows-legacyhive-zero-day-unofficial-patch`
- **Must-know:** no
...
- **Slug:** `jadepuffer-encforge-ransomware-ai-infrastructure`
- **Must-know:** no
- **Summary:** Sysdig linked a second Langflow RCE attack to AI-agent-driven threat actor JADEPUFFER, now deploying ENCFORGE, a Go-based ransomware built specifically to encrypt model weights, vector indexes, and training datasets. The campaign shows ransomware operators increasingly targeting AI/ML assets directly.
...
- **Slug:** `ostium-23-7m-crypto-theft-off-chain-attack`
- **Must-know:** no
- **Summary:** Attackers stole $23.75 million from the Ostium trading platform's liquidity provider vault by compromising off-chain infrastructure that feeds price data into the protocol. The attack targeted supporting oracle infrastructure rather than smart contract logic directly.
...
- **Slug:** `aws-guardduty-investigation-agent`
- **Must-know:** no
- **Summary:** AWS launched the Amazon GuardDuty investigation agent in public preview, an AI-powered feature that automatically investigates GuardDuty findings across AWS environments. AWS says it cuts investigation time from hours to minutes.
...
- **Slug:** `ai-coding-agents-sandbox-escapes`
- **Must-know:** no
- **Summary:** Researchers escaped the sandboxes of Cursor, Codex, Gemini CLI, and Antigravity by having the AI agent write files that trusted host tools later executed outside the sandbox. Multiple CVEs were assigned and patched; Google downgraded two Antigravity findings.
...
- **Severity:** informational
- **Tags:** `llm`, `mcp`
- **Slug:** `mcp-session-handling-update`
- **Must-know:** no
- **Summary:** The Model Context Protocol is moving to a looser, "stateless" approach to server-side session IDs, closer to how ordinary websites already handle sessions. The change is meant to reduce implementation complexity for MCP server operators.
...
- **Slug:** `ivanti-llm-vulnerability-remediation`
- **Must-know:** no
- **Summary:** Ivanti CSO Daniel Spicer described the company's use of LLMs to automate vulnerability remediation, citing surprising early effectiveness from frontier models. Cost at scale and human-in-the-loop practicality remain open questions.
