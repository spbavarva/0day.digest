...
- **Slug:** `armored-likho-still-toolkit-telegram`
- **Must-know:** no
- **Summary:** Kaspersky detailed a new Armored Likho campaign using fundraising lures to deliver an updated Still Toolkit that steals Telegram data and eavesdrops on victims. No victim scale or sector was disclosed.
...
- **Slug:** `belgium-eid-authentication-rce`
- **Must-know:** no
- **Summary:** Severe vulnerabilities in a browser extension underlying Belgium's eID trust framework allowed remote code execution against citizen accounts. Highlights browser extensions as a weak link in otherwise strong authentication systems.
...
- **Slug:** `sharepoint-auth-bypass-cve-2026-55040-exploited`
- **Must-know:** no
- **Summary:** Attackers are exploiting CVE-2026-55040 (CVSS 9.1), a SharePoint auth bypass patched in July 2026, after PoC code went public. Admins should confirm patches are applied.
...
- **Slug:** `deepseek-v4-pro-0813-release`
- **Must-know:** no
- **Summary:** DeepSeek's V4 Pro 0813 model is available via OpenRouter with no official announcement page yet. Open weights are unconfirmed but plausible given past release patterns.

### 5. "City-Forum" Data-Theft Campaign Targets Salesforce, ServiceNow Portals
- **Source:** BleepingComputer — https://www.bleepingcomputer.com/news/security/city-forum-data-theft-attacks-target-salesforce-servicenow-portals/ (also covered by Dark Reading)
- **Section:** Cybersecurity — Primary
- **Severity:** high
- **Tags:** `data-breach`, `cloud-security`, `appsec`
- **Slug:** `city-forum-data-theft-salesforce-servicenow`
- **Must-know:** no
- **Summary:** The "City-Forum" campaign has used custom tooling since at least March 2025 to steal data exposed to anonymous users via Salesforce Experience Cloud and ServiceNow portals, hitting organizations across sectors.
...
- **Severity:** high
- **Tags:** `malware`
- **Slug:** `android-windrelay-spynote-nfc-malware`
- **Must-know:** no
- **Summary:** A new Android NFC relay malware, WindRelay, is paired with the SpyNote RAT to take out fraudulent loans and relay live credit card data to attackers in real time.
...
- **Slug:** `aws-iam-role-manager-launch`
- **Must-know:** no
- **Summary:** AWS introduced IAM Role Manager to give teams a better default starting point for creating IAM roles, aiming to reduce overly broad permissions from the outset rather than relying on reactive cleanup.
...
- **Severity:** high
- **Tags:** `vulnerability`, `cve`, `appsec`
- **Slug:** `adobe-commerce-cve-2026-71362-exploited`
- **Must-know:** no
...
- **Severity:** medium
- **Tags:** `malware`, `appsec`
- **Slug:** `fake-chrome-vpn-extensions-proxy-campaign`
- **Must-know:** no
...
- **Some Claude users are mad that Anthropic's new watermarks will catch them using it at their jobs, classes** — TechCrunch AI. Social-media backlash story with no technical or security substance beyond noting the watermark feature exists.
...
- **alchemy-utils 0.1a0** — Simon Willison. Personal side-project library experiment (database-agnostic sqlite-utils prototype), not significant enough for a digest post.
