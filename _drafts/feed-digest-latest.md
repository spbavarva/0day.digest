...
- **Summary:** Indirect prompt injection hidden in an apparently harmless repository can cause Claude Code to spawn a reverse shell on the developer's machine, giving an attacker remote access without the developer running anything directly.
...
- **Severity:** high
- **Tags:** `data-breach`
- **Summary:** The National Association of Insurance Commissioners was breached via its Oracle PeopleSoft deployment; the ShinyHunters extortion group claims to have stolen 3.1TB of data.
...
- **Severity:** high
- **Tags:** `phishing`
- **Summary:** Infoblox found more than 236,000 sites built on the legitimate DCloud Uni-App framework running investment-scam templates powering fake crypto exchanges, pig-butchering scams, and WhatsApp phishing.
...
- **Summary:** China-aligned group Mustang Panda is running campaigns against Indian government and hydropower targets, deploying new malware and abusing Zoho WorkDrive as a C2 channel; Acronis found active compromises in Indian government networks.
...
- **OpenAI is teasing new hardware for Codex** — The Verge AI. Marketing teaser for an unreleased device, no real product details yet.
...
- **DiScoFormer: One transformer for density and score, across distributions** — Hugging Face Blog. Niche research paper with no summary detail to report factually.
- **WhatsApp rolls out usernames to help users hide their phone number** — BleepingComputer / The Hacker News / SecurityWeek (3x duplicate). Consumer privacy feature rollout, not a vulnerability/attack story.
- **Arena, the AI leaderboard everyone uses, is now a $100M business** — TechCrunch AI. Generic business news, no security/model-launch substance.
- **Microsoft extends Windows Server 2022 hotpatching until October 2027** — BleepingComputer. Lifecycle/support extension, not a vulnerability or attack.
...
- **Cloud CISO Perspectives: How Google Cloud Security uses AI internally** — Google Cloud Security. Newsletter intro, no concrete news event.
...
- **US seizes hundreds of FIFA World Cup illegal streaming domains** — BleepingComputer. Duplicate of the Record's World Cup streaming takedown story.
- **Mapping Europe's AI Workforce Opportunity** — OpenAI Blog. Labor-market/economic report, no security or capability substance.
- **'DirtyClone' Linux Kernel Vulnerability Leads to Root Access** — SecurityWeek. Already drafted in the pending, unmerged PR `digest/2026-06-29-1155` from earlier today — skipped to avoid duplicate posts.
