...
- **Summary:** The Shai-Hulud infostealer source code, which leaked last week, has been repackaged into malicious npm packages targeting developer credentials. Demonstrates the rapid weaponization cycle from leaked malware to active supply chain attack.
...
- **Summary:** Three coordinated campaigns hit npm, PyPI, and Docker Hub within 48 hours, targeting API keys, cloud credentials, SSH keys, and tokens from developer workstations and CI/CD pipelines. Developer access is now explicitly the primary target.
...
- **Summary:** SHub Reaper impersonates Apple, Google, and Microsoft simultaneously in one macOS attack chain, bypasses Terminal mitigations, steals credentials, and installs a persistent backdoor. Full kill chain detail from SentinelOne.
...
- **Summary:** Four OpenClaw vulnerabilities chained to achieve credential theft, sandbox escape, and persistent backdoor. Patches available; apply immediately especially in deployments adjacent to sensitive infrastructure.
...
- **Summary:** Five enterprise vendors patched in a coordinated disclosure. Top priority is CVE-2026-8043 in Ivanti Xtraction (CVSS 9.6). Fortinet, SAP, VMware, and n8n also fixed auth bypass, SQLi, RCE, and privilege escalation.
...
- **Summary:** Several healthcare data breaches impacting hundreds of thousands to millions of US patients have been added to the HHS HIPAA portal. Affected organizations span hospital networks and healthcare service providers.
...
- **Summary:** Iranian threat actors have expanded OT/ICS targeting to include internet-exposed ATG systems at fuel facilities. ATG tampering can trigger false alarms, forced shutdowns, or physical overflow incidents.
...
- **Summary:** SentinelOne red-teamed a government education AI chatbot, escalating from social engineering to tunneling attacks. AI content-layer defenses don't protect underlying infrastructure from traditional exploitation techniques.
...
- **Summary:** Kaspersky Q1 2026 mobile stats highlight new SparkCat variants (OCR-based crypto wallet stealer) and updated Triada Android banking trojan. Both show continued active development.
...
- **Summary:** OpenAI and Dell partnered to bring Codex AI coding agents to hybrid and on-premise enterprise environments, enabling AI-assisted development for organizations with data residency or compliance constraints.

## Skippable

- **Grafana refuses to pay ransom after codebase theft** — The Record. Duplicate of BleepingComputer item with better technical detail (stolen GitHub token vector).
- **Elon Musk loses his case against Sam Altman** — The Verge AI. Legal verdict; no security or AI technology angle.
- **Elon Musk has lost his lawsuit against Sam Altman and OpenAI** — TechCrunch AI. Duplicate coverage of same verdict.
...
- **Amazon's new Alexa+ powered feature can generate podcast episodes** — TechCrunch AI. Duplicate of Alexa podcast item above.
...
- **The Open Agent Leaderboard** — Hugging Face Blog. Generic AI benchmarking tool; no summary, no security angle.
- **⚡ Weekly Recap: Exchange 0-Day, npm Worm, Fake AI Repo, Cisco Exploit** — The Hacker News. Roundup; individual items covered separately in this digest.
...
- **IT threat evolution in Q1 2026. Non-mobile statistics** — Securelist. General PC/IoT quarterly stats; mobile report covers the headline malware families.
- **Microsoft testing adjustable taskbar, Start menu in Windows 11** — BleepingComputer. Generic Windows UI feature; no security angle.
